...
# Get length data
sizepop_goa_dusky  <- sqlQuery(channel,paste("SELECT * FROM 
                      GOA.SIZECOMP_TOTAL WHERE 
                      (((GOA.SIZECOMP_TOTAL.SPECIES_CODE)=",species,"))",sep=""),
                      believeNRows=FALSE)

# Get age data
agepop_goa_dusky   <- sqlQuery(channel,paste("SELECT * FROM 
                      GOA.AGECOMP_TOTAL WHERE 
                      (((GOA.AGECOMP_TOTAL.SPECIES_CODE)=",species,"))",sep=""),
                      believeNRows=FALSE)

# Get CPUE data
CPUE_goa_dusky     <- sqlQuery(channel,paste("SELECT * FROM 
                      GOA.CPUE WHERE 
                      (((GOA.CPUE.SPECIES_CODE)=",species,"))",sep=""),
                      believeNRows=FALSE)

# Get length frequency data
lfreq_goa_dusky    <- sqlQuery(channel,paste("SELECT * FROM 
                      (RACEBASE.HAUL INNER JOIN RACEBASE.LENGTH ON RACEBASE.HAUL.HAULJOIN = RACEBASE.LENGTH.HAULJOIN) 
                      INNER JOIN GOA.BIENNIAL_SURVEYS ON RACEBASE.LENGTH.CRUISEJOIN = GOA.BIENNIAL_SURVEYS.CRUISEJOIN WHERE 
                      (((RACEBASE.LENGTH.REGION)='GOA') AND 
                      ((RACEBASE.LENGTH.SPECIES_CODE)=",species,") AND 
                      ((RACEBASE.HAUL.PERFORMANCE)>=0))",sep=""),
                      believeNRows=FALSE)

# Get specimen data
specimen_goa_dusky <- sqlQuery(channel,paste("SELECT * FROM 
                      (RACEBASE.HAUL INNER JOIN RACEBASE.SPECIMEN ON RACEBASE.HAUL.HAULJOIN = RACEBASE.SPECIMEN.HAULJOIN)
                      INNER JOIN GOA.BIENNIAL_SURVEYS ON RACEBASE.SPECIMEN.CRUISEJOIN = GOA.BIENNIAL_SURVEYS.CRUISEJOIN WHERE 
                      (((RACEBASE.SPECIMEN.SPECIES_CODE)=",species,") AND 
                      ((RACEBASE.SPECIMEN.REGION)='GOA'))",sep=""),
                      believeNRows=FALSE)
...
library(dplyr)
# specimen has more hauls than lfreq (1260 vs. 659), so need to semi_join both to have matching tows
# length(unique(lfreq.match$HAULJOIN))

lfreq.match <- lfreq %>%
                  semi_join(.,specimen,by="HAULJOIN") %>%
                  select(.,one_of(c("CRUISEJOIN","HAULJOIN","STRATUM","SPECIES_CODE",
                                    "LENGTH","FREQUENCY","SEX","SURVEY","YEAR")))

specimen.match <- specimen %>% 
                    semi_join(.,lfreq,by="HAULJOIN") %>%
                    select(.,one_of(c("CRUISEJOIN","HAULJOIN","STRATUM","SPECIES_CODE",
                                      "LENGTH","SEX","WEIGHT","AGE","SURVEY","YEAR")))
# Get data parameters together
species  <- sort(unique(specimen.match$SPECIES_CODE))
yrs      <- sort(unique(specimen.match$YEAR))
sex      <- sort(unique(specimen.match$SEX))

### Loop thru species ###
for(sp in 1:length(species)){
  
  # split dfs by species
  specimen_sp    <- subset(specimen.match,specimen.match$SPECIES_CODE==species[sp])
  lfreq_sp       <- subset(lfreq.match,lfreq.match$SPECIES_CODE==species[sp])
  ...
  for(y in 1:length(yrs)){
    ...
    tows     <- sort(unique(specimen_sp_y$HAULJOIN)) 
    
    ### Loop through tows
    for(t in 1:length(tows)){
      #split dfs by tow
      specimen_sp_y_t <- subset(specimen_sp_y,specimen_sp_y$HAULJOIN==tows[t])
      lfreq_sp_y_t    <- subset(lfreq_sp_y,lfreq_sp_y$HAULJOIN==tows[t])
      # sizepop_sp_y_t     <- subset(sizepop_sp_y,sizepop_sp_y$HAULJOIN==tows[t])
      # agepop_RACE_sp_y_t <- subset(agepop_RACE_sp_y,agepop_RACE_sp_y$HAULJOIN==tows[t])
    
    ### Loop thru sex ###
    for(sx in 1:length(sex)){
      
      # split dfs by sex
      specimen_sp_y_t_sx<-subset(specimen_sp_y_t,specimen_sp_y_t$SEX==sex[sx])
      # agepop_RACE_sp_y_t_sx<-subset(agepop_RACE_sp_y_t,agepop_RACE_sp_y_t$SEX==sex[sx])
      ...
      # if(length(specimen_sp_y_t_sx$SEX) == 0) {
      ...
      # if(sx==3)
      #   specimen_sp_y_t_sx <- specimen_sp_y_t
      # 
      # # If there is no sizecomp data, we are wasting our time
      # if(sex[sx] == 1 & sum(sizepop_sp_y_t$MALES)==0) {
      #   cat(paste("No sizecomp data for sex", sx, "\n"))
      #   next
      # }
      # if(sex[sx] == 2 & sum(sizepop_sp_y_t$FEMALES)==0) {
      #   cat(paste("No sizecomp data for sex", sx, "\n"))
      #   next
      # }
      # if(sex[sx] == 3 & sum(sizepop_sp_y_t$UNSEXED)==0) {
      #   cat(paste("No sizecomp data for sex", sx, "\n"))
      #   next
      # }
      
      # Get vector of possible lengths
      lenlist<-seq(from = min(lfreq_sp_y_t$LENGTH), to = max(lfreq_sp_y_t$LENGTH), by = 10)
      # Get number of ages by length and age
      age_num <- tapply(specimen_sp_y_t_sx$AGE, list(specimen_sp_y_t_sx$LENGTH, specimen_sp_y_t_sx$AGE), length)
      ...
      no.lengths <- unique(sort(specimen_sp_y_t_sx$LENGTH))[is.na(match(unique(sort(specimen_sp_y_t_sx$LENGTH)), 
                                                                        lfreq_sp_y_t$LENGTH))]
      # Find lengths from sizecomp data where there is no age data.
      no.ages <- lfreq_sp_y_t$LENGTH[is.na(match(lfreq_sp_y_t$LENGTH, unique(specimen_sp_y_t_sx$LENGTH)))]
      if(length(no.ages) == 0)
        no.ages <- lfreq_sp_y_t$LENGTH
      no.age.sizecomp <- lfreq_sp_y_t[match(no.ages, lfreq_sp_y_t$LENGTH),]
      ...
      # sizepop_sp_y_t is a df of numbers at size, separated by sex for each year and tow
      if(sex[sx] ==1)
        pop_age_est_M <- age_frac * lfreq_sp_y_t$MALES[match(as.numeric(dimnames(age_frac)[[1]]), 
                                                             as.numeric(lfreq_sp_y_t$LENGTH), 
                                                             nomatch = 0, incomparables = no.lengths)]
      if(sex[sx] ==2)
        pop_age_est_F <- age_frac * lfreq_sp_y_t$FEMALES[match(as.numeric(dimnames(age_frac)[[1]]), 
                                                               as.numeric(lfreq_sp_y_t$LENGTH), 
                                                               nomatch = 0, incomparables = no.lengths)]
      if(sex[sx] ==3)
        pop_age_est_U <- age_frac * lfreq_sp_y_t$UNSEXED[match(as.numeric(dimnames(age_frac)[[1]]), 
                                                               as.numeric(lfreq_sp_y_t$LENGTH), 
                                                               nomatch = 0, incomparables = no.lengths)]
    ...
    if(exists("pop_age_est_M")==TRUE){
      if(length(pop_age_est_M)>0){
        ...
        agepop_RACE_sp_y_t_M <- subset(agepop_RACE_sp_y_t,agepop_RACE_sp_y_t$SEX==sex[1])
        ...
        for(i in 1:length(test_M[,3])){
          if(length(match(test_M[i,1],agepop_RACE_sp_y_t_M$AGE))>0)
            test_M[i,3]<-agepop_RACE_sp_y_t_M$AGEPOP[match(test_M[i,1],agepop_RACE_sp_y_t_M$AGE)]
        }
    ...
    if(exists("pop_age_est_F")==TRUE){if(length(pop_age_est_F)>0){
      ...
      agepop_RACE_sp_y_t_F<-subset(agepop_RACE_sp_y_t,agepop_RACE_sp_y_t$SEX==sex[2])
      ...
      for(i in 1:length(test_F[,3])){
        if(length(match(test_F[i,1],agepop_RACE_sp_y_t_F$AGE))>0)
          test_F[i,3]<-agepop_RACE_sp_y_t_F$AGEPOP[match(test_F[i,1],agepop_RACE_sp_y_t_F$AGE)]
      }
    ...
    if(exists("pop_age_est_U")==TRUE){if(length(pop_age_est_U)>0){
      ...
      agepop_RACE_sp_y_t_U<-subset(agepop_RACE_sp_y_t,agepop_RACE_sp_y_t$SEX==sex[3])
      ...
      for(i in 1:length(test_U[,3])){
        if(length(match(test_U[i,1],agepop_RACE_sp_y_t_U$AGE))>0)
          test_U[i,3]<-agepop_RACE_sp_y_t_U$AGEPOP[match(test_U[i,1],agepop_RACE_sp_y_t_U$AGE)]
      }
  ...
  # Test if there's disagreement with RACE calcs
  # (larger than 5% for the maximum difference at age; some years that are different with RACE that are unexplainable)
